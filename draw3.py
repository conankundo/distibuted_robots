...
class DRAW:
    ...
    def plot(self):
        ...
        init_points = [6, 5, 8, 15, 25, 13, 16,18,22, 387, 384, 385, 386, 390, 388, 389, 392, 393, 394, 398, 396]
        # #điểm đích
        ...
        while running:
            ...
            for robot in robots:
                ...
                for other_robot in robots:
                    if robot != other_robot:
                        # if len(other_robot.path) > 2 and len(robot.path) > 2 and robot.path[1] == other_robot.path[1]:
                        #     robot.status = 3
                        ...
                        #     if other_robot.prev_goal in robot.path_wait and robot.prev_goal not in other_robot.path_wait:
                        #         other_robot.status = 4
                        #         other_robot.path = other_robot.path_wait
                        #     elif robot.prev_goal in other_robot.path_wait and other_robot.prev_goal not in robot.path_wait:
                        #         robot.status = 4
                        #         robot.path = robot.path_wait
                        #     # elif len(robot.path_wait) <= len(other_robot.path_wait):
                        #     #     robot.status = 4
                        #     #     robot.path = robot.path_wait
                        #     # elif len(robot.path_wait) > len(other_robot.path_wait):
                        #     #     other_robot.status = 4
                        ...
                        #     #     other_robot.status = 4
                        #     #     other_robot.path = other_robot.path_wait
                        # elif len(robot.path) > 1 and len(other_robot.path) == 0 and robot.path[1] == other_robot.prev_goal:
                        #     robot.status = 3
                        #     robot.path_wait = robot.path
                        #     robot.path = []
                        #     other_robot.status = 4
                        ...
                        # elif len(robot.path) == 0 and len(other_robot.path) > 1 and other_robot.path[1] == robot.prev_goal:
                        #     robot.status = 4
                        #     robot.path = robot.path_wait
                        #     other_robot.path_wait = other_robot.path
                        #     other_robot.status = 3
                        #     other_robot.path = []
            ##########################################################
                        if len(other_robot.path) > 2 and len(robot.path) > 2 and (robot.path[1] == other_robot.path[1] or robot.path[0] == other_robot.path[0]):
                            robot.status = 3
                            ...
                            # elif robot.path_wait[0] in other_robot.path_wait and other_robot.path_wait[0] not in robot.path_wait:
                            #     robot.status = 4
                            #     robot.path = robot.path_wait
                            ...
                            # else:
                            #     other_robot.path_wait.insert(0, other_robot.prev_goal)
                            #     other_robot.status = 4
                            #     other_robot.path = other_robot.path_wait
                        ...
                        if robot.status != 3 and other_robot.status == 3 and len(robot.path) > 1 and robot.path[0] == other_robot.prev_goal:
                            robot.status = 3
                            robot.path_wait = robot.path.copy()
                            robot.path = []
                            other_robot.status = 4
                        ...
                        elif robot.status == 3 and other_robot.status != 3 and len(other_robot.path) > 1 and  other_robot.path[0] == robot.prev_goal and robot.path_wait[0] != other_robot.path[1]:
                            robot.status = 4
                            robot.path = robot.path_wait.copy()
                            other_robot.status = 3
                            other_robot.path_wait = other_robot.path.copy()
                            other_robot.path = []
                        elif robot.status == 3 and  other_robot.status == 3 and len(robot.path_wait) > 0 and robot.path_wait[0] not in other_robot.path_wait:
                            robot.status = 4
                            robot.path = robot.path_wait.copy()
                        elif robot.status == 3  and other_robot.status == 3 and len(other_robot.path_wait) > 0 and other_robot.path_wait[0] not in other_robot.path_wait:
                            other_robot.status = 4
                            other_robot.path = other_robot.path_wait.copy()
                        else:
                            robot.status = robot.status
                ###########################################################
                        # elif len(robot.path) > 1 and len(other_robot.path) > 1 and robot.path[1] == other_robot.path[1]:
                        #     if len(robot.path) < len(other_robot.path):
                        #         other_robot.status = 3
                        #         other_robot.path_wait = other_robot.path_wait
                        #         other_robot.path = []
                        #         robot.status = 3
                        #         robot.path_wait = robot.path_wait
                        #         robot.path = []
                        # else:
                        #     robot.status = robot.status
            ...
            # print(status)
                print(i,"status ",robots[i].status," pre_goal ",robots[i].prev_goal,": path: ", robots[i].path," path_wait", robots[i].path_wait)
